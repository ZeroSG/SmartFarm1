...
class _DemandState extends State<Demand> {
  ...
  List<dynamic> nowresult1_233 (List<dynamic> lat1){
    var seen = Set<String>();
        List uniquelist = lat1.where((student) => seen.add(student['c_order'])).toList();
    nowresult1_23 =  uniquelist.map((e) => {
           'c_order' : e['c_order'],
           'c_formula' : e['c_formula'],
           'house': lat1.where((element) => element['c_order'].toString().compareTo(e['c_order'].toString()) == 0).toList(),
  ...
  List<dynamic> nowresult1_2333 (List<dynamic> lat1,index){
    nowresult1_23 =  lat1.map((e) => {
           'c_order' : e['c_order'],
           'c_formula' : e['c_formula'],
           'house': lat1[index]["List"].where((element) => element['c_order'].toString().compareTo(e['c_order'].toString()) == 0).toList(),
  ...
  Future<void> getjaon1_demand_information() async {
    try {
        var urlsum1 = Uri.https("smartfarmpro.com", "/v1/api/setting/setting-farm");
  ...
  "Date_End": "$Y-$M-${_selectedValue.day}"
      ...
      if (ressum.statusCode == 200) {
       ...
       if(nowresult1_1[0]["n_refill"].split(' ').last == 'Bag'){
            var seen = Set<String>();
        List uniquelist = nowresult1_1.where((student) => seen.add(student['c_order'])).toList();
      ...
      setState(() {
       ...
       List<dynamic>   nowresult1_2 = uniquelist.map((e) => {
    ...
    return {"Numder": 0,'c_order':'','c_formula':'','List': []};
  });
        ...
        // print('====A ${_products1}');
         for(int j = 0;j<_products1.length;j++){
          for(int i = S;i<nowresult1_1.length;i++){
             if((_products1[j]["Numder"]<(nowresult1_21[0]['n_weight']*nowresult1_21[0]['n_box']))&&((_products1[j]["Numder"]+int.parse('${nowresult1_1[i]["n_refill"].split(' ').first.replaceAll(',', '')}'))<=(nowresult1_21[0]['n_weight']*nowresult1_21[0]['n_box']))){
               _products1[j]["List"] += [nowresult1_1[i]];
                ...
                _products1[j]["c_formula"] = nowresult1_1[S]['c_formula'];
                    _products1[j]["Numder"] += int.parse('${nowresult1_1[i]["n_refill"].split(' ').first.replaceAll(',', '')}') ; 
      ...
      //   .toList();
        ...
        List<dynamic>   nowresult3_42_ = _products1.where((x) =>( x['Numder'])!= 0).toList();
        now1 = nowresult3_42_;
      }
      print('====A ${now1}');
...
//  print('====${nowresult3_42_}');
         ...
         for(int j = 0;j<_products2.length;j++){
          for(int i = S1;i<nowresult3_42_.length;i++){
             if((_products2[j]["Numder"]<((nowresult1_21[0]['n_weight']*nowresult1_21[0]['n_box'])*nowresult1_21[0]['n_tow']))&&((_products2[j]["Numder"]+nowresult3_42_[i]['Numder'])<((nowresult1_21[0]['n_weight']*nowresult1_21[0]['n_box'])*nowresult1_21[0]['n_tow']))){
              //  print('====${ _products2}'); 
       ...
       List<dynamic>   nowresult3_42_1 = _products2.where((x) =>( ( x['Numder'])!= 0)).toList();  
                       ...
                       child: Column(children: [
                         Padding(
                           ...
                           child: Row(
                             mainAxisAlignment: MainAxisAlignment.spaceBetween,
                             children: [
                               Text('Order : ${nowresult12_1[index]['c_order']}',
                              style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 17,
                                  fontFamily: 'Montserrat',
                                  color: Color(0xff44bca3)),),
                               Text('No.${nowresult12_1[index]['c_formula']}',
                              style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 17,
                                  fontFamily: 'Montserrat',
                ...
                itemBuilder: (BuildContext context, int index1){
                    return Container(
                      child: Row(
                       mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                            Column(
                              children: [
                                Text("${nowresult12_1[index]['house'][index1]['c_house']??'OVER'}",
                              style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 15,
                                  fontFamily: 'Montserrat',
                                  color: C[index1]),),
                                 Text("${nowresult12_1[index]['house'][index1]['c_silo']??''}",
                              style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 15,
                                  fontFamily: 'Montserrat',
                            ...
                            Text('Day${nowresult12_1[index]['house'][index1]['n_day']??''}',
                              style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 15,
                                  fontFamily: 'Montserrat',
                             ...
                             Column(
                               children: [
                                 Text(
                                    ...
                                    Text('Filling Date:${nowresult12_1[index]['house'][index1]['d_gdate']}',
                              style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 15,
                                  fontFamily: 'Montserrat',
                     ...
                     Row(
                           ...
                           nowresult12_1[index]['c_house']== null ?  Container(
                               width: 380,
                              //  width: screenW*0.8,
                               height: 220,
                                child: Center(
                    ...
                    style: TextStyle(fontSize: 18),
                        ...
                        child: Center(
                          child: Container(
                                margin: EdgeInsets.only(top: 10),
                                child: Image(
                                  image: AssetImage('images/sacks3.png',),
                                  width: 200,
                                  //  width: screenW*0.7,
                                 height: 170,
                                  fit: BoxFit.fill,),
                            ...
                            child: Column(children: [
                              Padding(
                                ...
                                child: ListView.builder(
                                    ...
                                    itemCount:
                                      //  now1[index]['List'].length,
                                       nowresult1_23[index0]['house'].length,
                                    itemBuilder:
                                              ...
                                              children: [
                                                Text(
                                                  ...
                                                  style: TextStyle(
                                                      ...
                                                      fontSize: 15,
                                                      fontFamily: 'Montserrat',
                                                      color: index0 == 0 ? C[index1]: C[index1+int.parse('${now1[index0]['List'].length}')]),
                                                ),
                                                Text(
     ...
     return Container(
                  child: ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index1){
                          ...
                          nowresult1_233(now2[index]["List"][index1]["List"]);
                                                  ...
                                                  children: [
                                                    Text(
                                                      ...
                                                      style: TextStyle(
                                                          ...
                                                          fontSize: 15,
                                                          fontFamily: 'Montserrat',
                                                          color:   index1 != 0 ? C[index2+int.parse('${now2[index]['List'][index1-1]['List'].length}')] :   index0 == 0 ? C[index2]: C[index2+int.parse('${now2[index0]['List'].length}')]),
                                                          // index1 == 0 ? C[index2]: 
    ...
    List<dynamic> color = [
     ...
     List<dynamic> nowresult1_22 = now1[index]['List']
        .map((e) => {
              ...
              'c_order': e['c_order'],
              'n_refill': int.parse(
                  e['n_refill'].split(' ').first.replaceAll(',', '')),
              'diff': e['diff'],
              'color': color
                      .where((element) =>
                          element['c_house']
  ...
  List<charts.Series<dynamic, String>> _createSampleDataBar2(int index,int A) {
    List<dynamic> color = [
      for (int i = 0; i < now2[index]['List'][A]['List'].length; i++)
        {'id': '$i','c_house': now2[index]['List'][A]['List'][i]['c_house'], 'color': A == 0 ? C[i]: C[i+int.parse('${now2[index]['List'][0]['List'].length}')]},
