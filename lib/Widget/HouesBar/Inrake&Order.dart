...
class Inrake_Order extends StatefulWidget {
  ...
  Inrake_Order(
      {Key? key,
      this.Token,this.User,this.Password,
      this.View,
      this.samount1,
      this.num,
      this.day,
...
class _Inrake_OrderState extends State<Inrake_Order> {
    ...
    if (numamount2 == 0 || numamount1 == numamount2) {
        ...
        if (samount1 == widget.samount1![j]['name']) {
          return [
            for (int i = 0; i < nowresult2_1[0].keys.length; i++)
              if (nowresult2_1[0].keys.elementAt(i) != nowresult2_1[0].keys.elementAt(0))
                charts.Series<dynamic, String>(
                  colorFn: (__, _) => charts.ColorUtil.fromDartColor(C[i]),
                  id: '${nowresult2_1[0].keys.elementAt(i)}',
                  data: nowresult2_1,
                  // measureOffsetFn : (__, _) => 3,
                  domainFn: (dynamic daily20, _) => daily20['n_day']??daily20['${nowresult2_1[0].keys.elementAt(0)}'].toString(),
      ...
      if(samount2 != samount1){
        ...
        if (samount1 == widget.samount1![j]['name']) {
          return [
            for (int i = 0; i < nowresult2_1[0].keys.length; i++)
              if (nowresult2[0].keys.elementAt(i) != nowresult2[0].keys.elementAt(0))
                charts.Series<dynamic, String>(
                  colorFn: (__, _) => charts.ColorUtil.fromDartColor(C[i]),
                  id: '${nowresult2[0].keys.elementAt(i)}',
                  data: nowresult2,
                  domainFn: (dynamic daily20, _) => daily20['n_day']??daily20['${nowresult2[0].keys.elementAt(0)}'].toString(),
                  measureFn: (dynamic daily20, _) =>
                      daily20['${nowresult2[0].keys.elementAt(i)}'] ?? null,
                )..setAttribute(charts.rendererIdKey, 'customLine'),
                
            for (int i = nowresult2_1[0].keys.length; i < nowresult2[0].keys.length; i++)
                charts.Series<dynamic, String>(
                  colorFn: (__, _) => charts.ColorUtil.fromDartColor(C[i]),
                  id: '${nowresult2[0].keys.elementAt(i)}',
                  data: nowresult2,
  ...
  List<Color> C = [
   Color.fromARGB(255, 255, 230, 0),Color.fromARGB(255, 255, 0, 0),Color.fromARGB(255, 131, 0, 116),Color.fromARGB(255, 255, 0, 221),Color.fromARGB(255, 24, 0, 179),
      Color.fromARGB(255, 200, 117, 164),Color.fromARGB(255, 234, 242, 162),Color.fromARGB(255, 222, 143, 214),Color.fromARGB(255, 164, 57, 148),Color.fromARGB(255, 81, 184, 81),Color.fromARGB(255, 255, 85, 0),Color.fromARGB(255, 125, 193, 202)
  ];
  bool loading1 = true;
  bool loading2 = true;
  bool loading3 = true;
        ...
        setState(() {
          ...
          loading2 = false;
           ...
           if(sPlot =='แสดงถึงวันปัจจุบัน'){
            ...
            var newDate = new DateTime(dateTime1_.year, dateTime1_.month - 8, dateTime1_.day-10);
            // print('nowresult2_1 ${nowresult2_1}');
            List<dynamic> _products1 = [];
           for(int i= 0;i<nowresult2_1.length;i++){
             var splitted = nowresult2_1[i]['${nowresult2_1[i].keys.elementAt(0)}'].split('(').first;
            ...
            //  print('_products2 ${splitted1[2]}');
             ...
             if((int.parse(splitted1[1])<dateTime1_.month)&&(int.parse("20${splitted1[2]}")==dateTime1_.year)){
             _products1 += [nowresult2_1[i]];
           
             }
             if((int.parse(splitted1[0])<=dateTime1_.day&&int.parse(splitted1[1])==dateTime1_.month)){
             _products1 += [nowresult2_1[i]];
          ...
          //  if(nowresult2_1)
           ...
           for(int i= 0;i<nowresult2_2.length;i++){
             var splitted = nowresult2_2[i]['${nowresult2_2[i].keys.elementAt(0)}'].split('(').first;
             var splitted1 = splitted.split('-');
         ...
         List<dynamic>   nowresult2_ = nowresult2_1.map((e) => {
    for (int i = 0; i < nowresult2_1[0].keys.length; i++)
    '${nowresult2_1[0].keys.elementAt(i)}' : e['${nowresult2_1[0].keys.elementAt(i)}'],
    for (int i = 1; i < nowresult2_2[0].keys.length; i++)
    '${nowresult2_2[0].keys.elementAt(i)}' : nowresult2_2.where((element) => element['${nowresult2_1[0].keys.elementAt(0)}'].toString().compareTo(e['${nowresult2_1[0].keys.elementAt(0)}'].toString()) == 0).first['${nowresult2_2[0].keys.elementAt(i)}']?? null,
        ...
        setState(() {
          ...
          if(codeday == 'Day')  {   
           ...
           for(int i= 0;i<nowresult3.length;i++){
             var splitted = nowresult3[i]['${nowresult3[i].keys.elementAt(0)}'].toString().split('(').first;
             var splitted1 = splitted.split('-');
            //  print('_products2 20${splitted1[2]}');
             if(int.parse("20${splitted1[2]}")<dateTime1_.year){
                ...
                children: [
                   Padding(
                     ...
                     fontWeight: FontWeight.bold,
                                 fontSize: 15, fontFamily: 'Montserrat', color: Color.fromARGB(255, 0, 0, 0)),),
                                   ),
                                  Container(
                       decoration: BoxDecoration(
                        ...
                        color: Color.fromARGB(255, 235, 235, 235)),
                         ...
                         builder: (context)=>   Drawer1(Token: widget.Token, num1: 2,User: user,Password: password,HOUSE1:widget.num,HOUSE2: widget.HOUSEname,cropnum1: widget.cropnum1,cropnum:widget.cropnum,cropnum2:widget.cropnum2,farmnum:widget.farmnum,Feed: widget.Feed,),), (route) => false);
                            ...
                            children: [
                              Center(
                                child: DropdownButtonHideUnderline(
                                  child: widget.Feed == null ? DropdownButton<String>(
                                      value: sDefault,
                                      items: Default.map(
                                          (Default) => DropdownMenuItem<String>(
                                              value: Default,
                                              child: Text(
                                                Default,
                                                style: TextStyle(
                                      ...
                                      value: sFeed1,
                                      items: widget.Feed!.map(
                                          (Feed1) => DropdownMenuItem<String>(
                                              value: Feed1["code"],
                                              child: Text(
                                                Feed1["name"],
                                                style: TextStyle(
                            ...
                            child: DropdownButtonHideUnderline(
                              child: widget.Feed == null ? DropdownButton<String>(
                                  ...
                                  : DropdownButton<String>(
                                    ...
                                    if(widget.Feed![i]['name'] == Feed1){
                                      print(widget.farmnum);
                                       print(widget.num);
                                        print(silo1);
                                        String? F = widget.Feed![i]['code'];
                                        print(F);
                                       
                                       API_edit_house_silo(widget.Token,widget.farmnum,widget.num,silo1.toStringAsFixed(0),F);
                                    }
                                  ...
                                  : DropdownButton<String>(
                                      ...
                                      onChanged: (Feed2) {
                                        setState(() {
                                          sFeed2 = Feed2!;
                                            for(int i = 0;i<widget.Feed!.length;i++){
                                    if(widget.Feed![i]['name'] == sFeed2){
                                       
                                       API_edit_house_silo(widget.Token,widget.farmnum,widget.num,silo2.toStringAsFixed(0),widget.Feed![i]['code']);
                                    }
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    Container(
                      margin: EdgeInsets.only(top: 5),
                      child: 
                         nowresult1_2[0]['c_remark'] != null ? Text(
...
Future<dynamic> DialogManual (BuildContext context,dynamic nowresult) {
  ...
  Death_Unit.text='${nowresult['n_death']==""?'0':nowresult['n_death']}';
  Reject_Unit.text='${nowresult['n_reject']==""?'0':nowresult['n_reject']}';
  Usage_Bag.text='${nowresult['n_bag']==""?'0.0':  double.parse( nowresult['n_bag'].split(' ').first) *1000}';
  Weight_Unit.text='${nowresult['n_weight']==""?'0.00':nowresult['n_weight'].split(' ').first}';
   AddOn_Unit.text='${nowresult['n_addon']==""?'0':nowresult['n_addon']}';

    
    Crop2 = int.parse('${nowresult['n_using_refill']==""?'0':nowresult['n_using_refill'].split('.').first}');

  Silo1Usage.text='${nowresult['n_usage_1']==""?'0.00':double.parse( nowresult['n_usage_1'].split(' ').first) *1000}';
  Silo1Remain.text='${nowresult['n_remain_1']==""?'0.00':double.parse( nowresult['n_remain_1'].split(' ').first) *1000}';
  

  Silo1Refill.text='${nowresult['n_refill_1']==""?'0.00':double.parse( nowresult['n_refill_1'].split(' ').first) *1000}';
  Silo2Usage.text='${nowresult['n_usage_2']==""?'0.00':double.parse( nowresult['n_usage_2'].split(' ').first) *1000}';
  Silo2Remain.text ='${nowresult['n_remain_2']==""?'0.00':double.parse( nowresult['n_remain_2'].split(' ').first) *1000}';


  Silo2Refill.text ='${nowresult['n_refill_2']==""?'0.00':double.parse( nowresult['n_refill_2'].split(' ').first) *1000}';
    return showDialog(
        ...
        Container(
                          ...
                          child: TextButton(
                            onPressed: () {
                             ...
                             API_edit_house_usage(widget.Token,widget.farmnum,widget.num,nowresult['id'], Date.text,
                             double.parse(Usage_Bag.text),int.parse(Death_Unit.text),int.parse(Reject_Unit.text), int.parse(AddOn_Unit.text),double.parse(Weight_Unit.text),
                             double.parse(Silo1Usage.text),double.parse(Silo2Usage.text),double.parse(Silo1Remain.text), double.parse(Silo2Remain.text),double.parse(Silo1Refill.text),double.parse(Silo2Refill.text),
                             Crop2);
                              if(widget.cropnum2 != null){
    getjaon1_house_information();
    }
...
borderRadius: BorderRadius.circular(5),
  color: Crop1==false ? Colors.white
...
child: TextField(
    onChanged: (text1) {
    Silo1Remain.text = '${double.parse(Silo1Refill.text==""?'0.00':Silo1Refill.text)-double.parse(Silo1Usage.text==""?'0.00':Silo1Usage.text)}' ;
  },
...
child: TextField(
    onChanged: (text1) {
    Silo2Remain.text = '${double.parse(Silo2Refill.text==""?'0.00':Silo2Refill.text)-double.parse(Silo2Usage.text==""?'0.00':Silo2Usage.text)}' ;
  },
...
child: TextField(
   onChanged: (text1) {
    Silo1Usage.text = '${double.parse(Silo1Refill.text==""?'0.00':Silo1Refill.text)-double.parse(Silo1Remain.text==""?'0.00':Silo1Remain.text)}' ;
  },
...
child: TextField(
  onChanged: (text1) {
    Silo2Usage.text = '${double.parse(Silo2Refill.text==""?'0.00':Silo2Refill.text)-double.parse(Silo2Remain.text==""?'0.00':Silo2Remain.text)}' ;
  },
              ...
              children: [
                ...
                Container(
                  ...
                  child: DataTable2(
                    ...
                    rows: widget.cropnum2 == null ?   _products1.map((item) {
                    return  DataRow(cells: [
                          DataCell(Center(child: Text(''))),
                          DataCell(Center(child: Text(''))),
                          DataCell(Center(child: Text(''))),
                    ...
                    child: Padding(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                            ...
                            value: nameday,
                            items: widget.day!.map((day) => DropdownMenuItem<String>(
                                value: day['name'],
                                child: Text(
                                  day['name'],
                                  style: TextStyle(
                    ...
                    child: Padding(
                      ...
                      child: DropdownButtonHideUnderline(
                        ...
                        ? DropdownButton<String>(
                            ...
                            value: Noname,
                            items: NoList!
                                .map((Nosamount2) => DropdownMenuItem<String>(
                                    value: Nosamount2,
                                    child: Text(
                                      Nosamount2,
                                      style: TextStyle(
                                ...
                                if (amount == widget.samount1![i]['name']) {
                                  ...
                                  if(amount.contains('ต่อตัว')){
                                    ...
                                    if(amount.contains('%')){
                                     ...
                                     if(!amount.contains('ต่อตัว')&&!amount.contains('ทั้งหมด')&&!amount.contains('อัตรา %')&&!amount.contains('ต่อวัน')){
                                      kg1 = '';
                                    }
                        ...
                        : DropdownButton<String>(
                            ...
                            onChanged: (samount22) {
                              for (int i = 0;
                                  i < widget.samount1!.length;
                                  i++) {
                                if (samount22 == widget.samount1![i]['name']) {
                                  setState(() {
                                    ...
                                    if(samount22.contains('อัตรา %')){
                                      ...
                                      if(!samount22.contains('ต่อตัว')&&!samount22.contains('ทั้งหมด')&&!samount22.contains('อัตรา %')&&!samount22.contains('ต่อวัน')){
                                      kg2 = '';
                                    }
                                    getjaon2_house_compare();
                                  });
      ...
      child: charts.BarChart(
        ...
        behaviors: [
          ...
          new charts.ChartTitle('day',
              ...
              titleOutsideJustification:
                  ...
                  titleStyleSpec: charts.TextStyleSpec(fontSize: 14,fontFamily: 'Montserrat',),),
                  new charts.ChartTitle(kg1,
            behaviorPosition: charts.BehaviorPosition.start,
            titleOutsideJustification:
                charts.OutsideJustification.middleDrawArea,
                titleStyleSpec: charts.TextStyleSpec(fontSize: 14,fontFamily: 'Montserrat',),
                ),
                 new charts.ChartTitle(kg2,
        ...
        domainAxis: charts.OrdinalAxisSpec(
          ...
          tickProviderSpec:
            ...
            new charts.TickSpec('${nowresult2_1[0]['n_day']?? nowresult2_1[0].keys.elementAt(0).toString()}'),
            ...
            if (i  >= 0 && i < nowresult2_1.length)
                new charts.TickSpec('${nowresult2_1[i][nowresult2_1[0].keys.elementAt(0).toString()]}'),   
          ]),
        ),
                ...
                if(samount1 == amount![j]['name']){
                 ...
                 model.selectedDatum.forEach((charts.SeriesDatum datumPair) {
                  selectedDatum!.add({
                    ...
                    'title': '${datumPair.datum['n_day']??datumPair.datum['${nowresult2_1[0].keys.elementAt(0)}'].toString()}',
                    for(int i = 1;i<nowresult2_1[0].keys.length;i++)
                    'subTitle${i}': '${datumPair.datum['${nowresult2_1[0].keys.elementAt(i)}'] ?? 'undefeated'}',
                    // })
                  });});}}}
                  if(samount2 != 'ไม่เลือกชุดข้อมูล'){
                       for(int j =0;j<widget.samount1!.length;j++){
                if(samount1 == amount![j]['name']){
                 model.selectedDatum.forEach((charts.SeriesDatum datumPair) {
                  selectedDatum!.add({  
                    ...
                    'title': '${datumPair.datum['n_day']??datumPair.datum['${nowresult2[0].keys.elementAt(0)}'].toString()}',
                    for(int i = 1;i<nowresult2[0].keys.length;i++)
                    'subTitle${i}': '${datumPair.datum['${nowresult2[0].keys.elementAt(i)}'] ?? 'undefeated'}',
                    // for(int i = 1;i<nowresult2_2[0].keys.length;i++)
  ...
  Widget build5(BuildContext context) => ExpansionTile(
        ...
        children: [
                        ...
                        child: DropdownButton<String>(
                            ...
                            onChanged: (Graph) {
                              ...
                              if (Graph == 'กราฟแท่งแบบเลื่อนได้') {
                                setState(() {
                                  if(nowresult3[0].keys.elementAt(0)=='n_day')
                                  num = 10;
                                  if(nowresult3[0].keys.elementAt(0)=='n_week')
                                  num = 3;
                                  if(nowresult3[0].keys.elementAt(0)=='n_month')
                                  num = 2;
                                });
                              }
                            }),
          ...
          Container(
              margin: EdgeInsets.only(top: 10),
              height: screenH * 0.57,
              child: widget.cropnum2 == null ? Container(
            height: screenH * 0.57,
            child: Center(child: Text('No data to display.',
             style: TextStyle(
              fontSize: 16, fontFamily: 'Montserrat'),)),
  ...
  charts.BarChart BarChart5() {
    ...
    return charts.BarChart(
                    ...
                    domainAxis: charts.OrdinalAxisSpec(
                      ...
                      viewport: new charts.OrdinalViewport(nowresult3[0]['n_day']??nowresult3[0]['${nowresult3[0].keys.elementAt(0)}'].toString(), num),
          ...
          if (i  >= 0 && i < nowresult3.length)
              new charts.TickSpec('${nowresult3[i][nowresult3[0].keys.elementAt(0).toString()]}'),       
                      ]),
                    ),
                    selectionModels: [
                      charts.SelectionModelConfig(
                        updatedListener: (charts.SelectionModel model) {
                          if (model.hasDatumSelection) {
                            ...
                            model.selectedDatum
                              ...
                              selectedDatum!.add({
                                date = '${datumPair.datum['n_day']??datumPair.datum['${nowresult3[0].keys.elementAt(0)}']}',
                                pointerValue1 =
                                ...
                                ToolTipMgr.setTitle({
                                  'title': '${datumPair.datum['n_day']??datumPair.datum['${nowresult3[0].keys.elementAt(0)}']}',
                                  'subTitle': '${datumPair.datum['${nowresult3[0].keys.elementAt(1)}'] ?? 'undefeated'}',
                                  'subTitle1': '${datumPair.datum['${nowresult3[0].keys.elementAt(2)}'] ?? 'undefeated'}',
                                  'subTitle2': '${datumPair.datum['${nowresult3[0].keys.elementAt(3)}'] ?? 'undefeated'}',
                                })
                              });
                            });
                          }
                    ...
                    behaviors: [
                      ...
                      new charts.SelectNearest(
                          eventTrigger:
                              charts.SelectionTrigger.longPressHold),
                      charts.LinePointHighlighter(
                        // defaultRadiusPx: 5.0,
                      ...
                      new charts.SeriesLegend(
                        cellPadding:
                            EdgeInsets.symmetric(horizontal: screenW * 0.02),
  ...
  List<charts.Series<dynamic, String>> _createSampleDataBar() {
    return [
      charts.Series<dynamic, String>(
        ...
        domainFn: (dynamic nowresult, _) => nowresult['n_day']??nowresult['${nowresult3[0].keys.elementAt(0)}'].toString(),
