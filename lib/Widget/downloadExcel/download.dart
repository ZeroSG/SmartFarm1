...
Future SetExcel(var excel,var EXcel) async {
    ...
    // Directory documentDiresctory = await getApplicationDocumentsDirectory();
     var documentDiresctory = await ExternalPath.getExternalStoragePublicDirectory(ExternalPath.DIRECTORY_DOWNLOADS);
    // String documentPath = documentDiresctory.path;
    Directory _appDocDirFolder = Directory("$documentDiresctory/");
...
if (status.isGranted) {
   ...
   String?  fileexcel ="$documentDiresctory/$EXcel${now.millisecond}${now.microsecond}.xlsx";
...
Sheet sheetObject = excel['Sheet1'];
    ...
    return {"Excel0_1": sheetObject.cell(CellIndex.indexByString("${A_Z[i]}1"))};
  });
  for(int j = 0;j<result[0].keys.length;j++){
    ...
    return {
      ...
      "Excel0_2_$t": sheetObject.cell(CellIndex.indexByString("${A_Z[t]}${i+2}"))};
  });
  for(int j = 0;j<result.length;j++){
      ...
      for(int t = 0;t<result[0].keys.length;t++){
        if(result[j]['${result[0].keys.elementAt(t)}']== null){
             ...
             Excel0_2[j]['Excel0_2_$t']!.value = '${result[j]['${result[0].keys.elementAt(t)}']}';
        }
